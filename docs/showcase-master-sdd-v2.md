...
- **Phase 0 (in-repo, no producer dependency):** run `validate:modqn:training-scene-trace-inventory` + read `inventory.ts` to enumerate, per source, what is `producer-backed` vs `source-gap`. Output: the exact channel list the dashboard may show now vs must source-gap. This replaces v1's green-field "define training-telemetry.json". **DONE 2026-06-02 → `docs/showcase-phase0-plane-scoping.md`** (channel→plane table, verified producer audit, cleanup reconciliation, P1a slice plan). Key finding: Plane C (`visual-showcase-v1`) backs the whole offline dashboard+flowchart today; Plane A live mid-run reward curve is a *gated* small producer change (G-A), not renderer work.
