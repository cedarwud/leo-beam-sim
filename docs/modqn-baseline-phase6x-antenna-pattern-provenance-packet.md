...
|---|---|---|---|
| `src/engine/signal/beam-gain.ts` | `runtime-current` | Live helper. **Corrected 2026-08-21**: now evaluates HOBS Eq.(3) as written — `J1(mu)/(2*mu) + 36*J3(mu)/mu^3` with `mu = 2.07123*sin(theta)/sin(theta_3dB)`, no renormalization (unity at boresight), floored at `-40 dB`. Until then it evaluated `2*J1(a)/a` with `alphaScale=1.835239914925094` and `BESSEL_J1_J3_BORESIGHT_ENVELOPE=1.75` — a transcription error plus two patches, with no paper, ITU-R, or vendored-source authority. | Leo's live HOBS/SINR antenna pattern now matches the paper and the vendored helper's convention. |
...
| `docs/modqn-baseline-phase6v-antenna-pattern-convention.md` | `validation/golden` | Phase 6V compared three conventions and found adapted same-theta residuals above tolerance. Status was `BLOCKED_BY_PATTERN_CONVENTION`; **resolved 2026-08-21** — the engine candidate was a transcription error, not a rival convention. Status is now `NEEDS_SOURCE_PROVENANCE`. | The J1+J3 convention question is closed (`antenna-model-mismatch: NOT_OBSERVED`). The remaining residual is the independent diameter/beamwidth mapping. |
| `scripts/fixtures/modqn-phase6v-antenna-pattern-convention.json` | `validation/golden` | Admits only candidates with explicit paper-backed, standard-backed, source-map, or source-code provenance. Expected status updated 2026-08-21 to `NEEDS_SOURCE_PROVENANCE`. | Blocks curve-fit or hidden adapter candidates. |
