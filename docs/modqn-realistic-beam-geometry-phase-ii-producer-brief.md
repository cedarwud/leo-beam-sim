...
- `K = maxActivePerSlot = min(layout.count, sats*beamsPerSat, requestedOverride)`. With override **28** and 37 cells → K=28 active, 9 idle, hopping preserved (SDD §4.2.1).
...
- Serving cap **L ∈ {4,8,12}** = top-L visible sats by elevation to area center (Phase I `useCellSchedule` selection; the consumer default L=8, demo selectable). Phase III sweeps L; Phase II just makes L a config knob.
...
- **II-S2 `env/cell_scheduler.py` + `tests/test_cell_scheduler.py`** — port §2.2. K=28, round-robin, visibility constraint, no double-beam, determinism, cross-lang parity vs TS. ~180 LOC.
...
- **II-S7 `configs/modqn-paper-baseline.realistic-cells.resolved.yaml`** — new track: `cells:{count:37}`, serving `L` knob (default 8; sweep set {4,8,12} for Phase III), `K=28`, `antenna.model: bessel-j1-j3`, `antenna.off_axis_enabled: true`. Retire `ASSUME-MODQN-REP-002` with an audit note pointing at the parent SDD §1.2/§2. Validate via `config_loader` round-trip.
