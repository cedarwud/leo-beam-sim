...
- **S-cells-4 — antenna truth-input + render scope + re-point (IN PROGRESS, split a–e).**
  ...
  - **(d) DONE** — the legacy `EarthFixedCells` 20-hex green-disc ground paint is RETIRED (`showEarthFixedCells` pinned false; MainScene mount + its dead hex-cover compute chain removed). It was a DIFFERENT layout competing with the cell-truth cones. The hex-cover MODULE + `validate:vc3a:hex-paint` logic gate are kept for reuse.
