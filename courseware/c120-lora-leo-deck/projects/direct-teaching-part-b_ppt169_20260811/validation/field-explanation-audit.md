...
| P040 | contact_open、steps_since_send、PACE_GAP_STEPS、REST_DURING_GAP | decision／command 與 baseline summary 分到 P040、P040a，避免 evidence 卡標題重疊。 |
| P041 | PACE_GAP_STEPS、REST_DURING_GAP、py_compile | P041 BEFORE；P041d AFTER；P041b commands；P041c marker 外 consumer／unique edit／interpret。 |
| P042 | baseline、candidate、hidden、freeze、result_path、replay | P042 只放六條 exact commands；P042a 分開 stdout fields、result／replay pair 與 freeze checkpoint。 |
...
| P047 | withheld、frozen policy、適用條件限制、claim ceiling | 卡片補中文定義與 frozen policy 狀態；band 說明適用範圍；不拆頁。 |
...
| P054 | A predecessor、B hold、Trace A、result_path | decision／command 與 Trace A baseline summary 分到 P054、P054a。 |
| P055 | ENTER_QUALITY、EXIT_QUALITY、STABLE_STEPS、marked block、py_compile | P055 BEFORE；P055d AFTER；P055b commands；P055c consumer／function location／unique edit。 |
| P056 | Trace A baseline、candidate、freeze、Trace B、result_path | P056 只放六條 exact commands；P056a 分開 stdout fields、result／replay pair 與 freeze checkpoint。 |
| P057 | quality band、MODE_CHANGE、PACKET_ATTEMPT、service_pass | P057 causal overview；P057a 獨立放 MODE_CHANGE、packet、service 與 energy evidence。 |
| P058 | A predecessor、B checkpoint、Trace B entry、receipt identity | 沿用 P046 定義；freeze lineage 卡只呈現三者關係與 entry 條件；不拆頁。 |
| P059 | Trace B、frozen policy、withheld、expired_packets | P059 frozen setup；P059a 獨立放 Trace B summary、service false 與 claim ceiling。 |
...
| P062 | policy lineage、checkpoint、py_compile、result_path、replay | P062 recovery overview；P062b 分開展示 WSL／PowerShell status 與支援 checkpoint commands。 |
