...
Evidence: VERIFIED（server bytes、checksum 內容與 listener）；EXTERNAL GET REHEARSAL PENDING
...
Notes: 請只從畫面上的兩個 URL 取得同名 ZIP 與 checksum。Linux 用 `sha256sum -c c120-lora-energy-lab-v1.zip.sha256`，macOS 用 `shasum -a 256` 後逐字比較，Windows PowerShell 用 `Get-FileHash .\\c120-lora-energy-lab-v1.zip -Algorithm SHA256`，再與畫面上的 64 位 hash 比對。這一步保護的是 artifact identity，不是 energy evidence。Server 端目前已驗證 listener、ZIP bytes 與 checksum 內容一致；controller 對外 HEAD 曾回 200，但完整 GET rehearsal 尚未穩定通過，因此不把它擴張成所有教室網路均已驗證。hash 不符就停止，不改用相似 archive。
...
- `VERIFIED / EXTERNAL GET REHEARSAL PENDING`：server listener、ZIP bytes 與 checksum 內容已核對；對外完整 GET 與教室網路路徑仍需 fresh rehearsal。Python 3.11 cold-start receipt、dependency lock/hash 與各平台 fresh-run 證據仍為 `PLACEHOLDER`。
