...
> 與 endpoint J；因此要先保住 service，再解釋能量差異。
...
可能改變哪個 action，再說要到哪個事件找證據。若結果相反，保留反例並解釋
中間狀態，不為了使答案好看而改預測。」
...
**讀法：**先核對 scenario／policy lineage，再讀 packet delivery 與兩個 gate，
...
## P071｜revision run 前：把「較早介入」寫成可反駁預測
...
預期回答：是 trade-off；先保留 service gate，再描述多花的 endpoint J。
...
## P074｜surprise withheld：不重調，使反例留下來

**主視覺：** frozen revision card 對上新條件的 withheld card。左側 policy hash
鎖定，右側用紅紫色 gate（不用亮橘文字）表達結果邊界；底部留一個反例問句。
...
withheld 當成反例，保存它的 result_path 與 provenance。

**不可下的結論：**不可說 margin 30 在所有窗口都有效，不可說低 J 就等於
成功，不可把 surprise 變成新的 candidate。

**構成約束：**一張 frozen policy 卡與一張 withheld 卡形成對照；反例句用 24–28pt，
數值標籤最低 18pt。
...
**完整句：**「先把 gate 讀成工作是否被保住，再問 endpoint J 為什麼
...
**完整句：**「先沿時間線指出 urgent card 何時生成、何時 attempt，再找
...
成功，還是保留這筆 withheld record？」預期回答：保留原 record，使反例成為
適用範圍的一部分。
...
**完整句：**「轉移內容為『觀察可用欄位 → 受控
action → 可回放事件 → service gate → endpoint J』這條可反駁機制。」
...
**完整句：**「先讀來源與 hash，再讀服務；檔名相似、案例名稱相同，都不
不取代這個 identity gate。」
...
## P088｜先追 queue 與 packet event，再回到摘要
...
先保留錯誤與原始 path，再回到 matching artifact 或 release backup，用新的
...
但它們的時間軸不保證同步；先說目前正在播放哪一層，再把相應的欄位接回
policy branch。」
