...
const TERM_METADATA: Readonly<Record<CanonicalTermKey, TermMetadata>> = Object.freeze({
  theta: { thesisSymbol: '\\theta_{u,b}', label: '偏軸角', unit: 'rad', target: 'link', dependencies: [] },
  transmitGain: { thesisSymbol: 'G^T_{u,b}', label: '發射天線增益', unit: 'linear', target: 'link', dependencies: ['theta'] },
  receiveGain: { thesisSymbol: 'G^R_{u,b}', label: '接收天線增益', unit: 'linear', target: 'link', dependencies: [] },
  largeScaleGain: { thesisSymbol: 'G^{LS}_{u,b}', label: '大尺度通道增益', unit: 'linear', target: 'link', dependencies: [] },
  ricianGain: { thesisSymbol: 'g_{u,b}', label: 'Rician 小尺度增益', unit: 'linear', target: 'link', dependencies: [] },
  rawH: { thesisSymbol: 'h_{u,b}', label: '原始通道功率增益', unit: 'linear', target: 'link', dependencies: ['transmitGain', 'receiveGain', 'largeScaleGain', 'ricianGain'] },
  hDiv: { thesisSymbol: 'h^{div}_{u}', label: '除法用通道增益', unit: 'linear', target: 'link', dependencies: ['rawH'] },
  minimumRate: { thesisSymbol: 'R_{min}', label: '每位使用者最低傳輸速率目標', unit: 'bit/s', target: 'system', dependencies: [] },
  beamLoad: { thesisSymbol: 'U_b', label: '波束服務使用者數', unit: 'users', target: 'beam', dependencies: [] },
  beamBandwidth: { thesisSymbol: 'B_{beam}', label: '每波束頻寬', unit: 'Hz', target: 'beam', dependencies: [] },
  gammaReq: { thesisSymbol: '\\gamma_{req}(U_b)', label: '需求 SINR', unit: 'linear', target: 'beam', dependencies: ['minimumRate', 'beamLoad', 'beamBandwidth'] },
  laggedInterference: { thesisSymbol: '\\widehat{I}_{u}', label: '前一狀態干擾估計', unit: 'W', target: 'link', dependencies: [] },
  pReqUser: { thesisSymbol: 'p^{req}_{u}', label: '使用者需求功率', unit: 'W', target: 'link', dependencies: ['gammaReq', 'hDiv', 'laggedInterference', 'noise'] },
  pReqBeam: { thesisSymbol: 'P^{req}_{b}', label: '波束需求功率', unit: 'W', target: 'beam', dependencies: ['pReqUser'] },
  beamPowerCap: { thesisSymbol: 'P_{beam,max}', label: '波束功率上限', unit: 'W', target: 'beam', dependencies: [] },
  satellitePowerCap: { thesisSymbol: 'P_{sat,max}', label: '衛星功率上限', unit: 'W', target: 'system', dependencies: [] },
  preSatelliteCapPower: { thesisSymbol: 'P^{DL}_{b}', label: '衛星上限前射頻需求', unit: 'W', target: 'beam', dependencies: ['pReqBeam', 'beamPowerCap'] },
  actualBeamRf: { thesisSymbol: '\\widetilde{P}^{DL}_{b}', label: '實際波束射頻輸出', unit: 'W', target: 'beam', dependencies: ['preSatelliteCapPower', 'satellitePowerCap'] },
  signal: { thesisSymbol: 'S_u', label: '接收訊號項', unit: 'W', target: 'link', dependencies: ['actualBeamRf', 'rawH'] },
  intraSatelliteInterference: { thesisSymbol: 'I^{intra}_u', label: '同衛星干擾', unit: 'W', target: 'link', dependencies: ['actualBeamRf', 'rawH'] },
  interSatelliteInterference: { thesisSymbol: 'I^{inter}_u', label: '跨衛星干擾', unit: 'W', target: 'link', dependencies: ['actualBeamRf', 'rawH'] },
  interference: { thesisSymbol: 'I_u', label: '總干擾', unit: 'W', target: 'link', dependencies: ['intraSatelliteInterference', 'interSatelliteInterference'] },
  noise: { thesisSymbol: '\\sigma^2', label: '雜訊功率', unit: 'W', target: 'link', dependencies: ['beamBandwidth'] },
  sinr: { thesisSymbol: '\\gamma_u', label: '實現 SINR', unit: 'linear', target: 'link', dependencies: ['signal', 'interference', 'noise'] },
  representativeRate: { thesisSymbol: 'R_u', label: '代表鏈路傳輸速率', unit: 'bit/s', target: 'link', dependencies: ['sinr', 'beamBandwidth', 'beamLoad'] },
  totalRate: { thesisSymbol: '\\sum_u R_u', label: '系統總吞吐量', unit: 'bit/s', target: 'system', dependencies: ['representativeRate'] },
  paEfficiency: { thesisSymbol: '\\eta^{PA}_{b}', label: '功率放大器效率', unit: 'linear', target: 'beam', dependencies: ['actualBeamRf', 'p0'] },
  paPower: { thesisSymbol: 'P^{PA}_{b}', label: '功率放大器直流功率', unit: 'W', target: 'beam', dependencies: ['actualBeamRf', 'paEfficiency'] },
  rfcPower: { thesisSymbol: 'P^{RFC}_{b}', label: '射頻鏈功率', unit: 'W', target: 'beam', dependencies: [] },
  basebandPower: { thesisSymbol: 'P^{BB}_{b}', label: '基頻功率分攤', unit: 'W', target: 'beam', dependencies: [] },
  eventPower: { thesisSymbol: 'P^{event}_{b}', label: '事件功率', unit: 'W', target: 'beam', dependencies: [] },
  systemPower: { thesisSymbol: 'P_{sys}', label: '模型邊界內系統功率', unit: 'W', target: 'system', dependencies: ['paPower', 'rfcPower', 'basebandPower', 'eventPower'] },
  eeInst: { thesisSymbol: 'EE_{inst}', label: '瞬時系統能效', unit: 'bit/J', target: 'system', dependencies: ['totalRate', 'systemPower'] },
  eeEval: { thesisSymbol: 'EE_{eval}', label: '區間 ratio-of-sums 能效', unit: 'bit/J', target: 'system', dependencies: ['totalRate', 'systemPower'] },
