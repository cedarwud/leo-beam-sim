...
const LOCAL_ZH: Record<string, string> = {
  ...
  'panel.formulaTerms.help': 'γ 由鏈路 RF 功率 p_{u,s,v}(t, θ_{u,s,v})、非角度通道因子 H_{u,s,v}(t)、角度相關發射增益 Gᵀ(θ_{u,s,v})、總同頻干擾 I_{u,s,v}(t, θ) 與接收端雜訊 σ² 共同計算。',
  ...
  'panel.formulaTerms.numerator.help': '分子為選定連線的接收訊號功率：由鏈路 RF 發射功率 p_{u,s,v}(t, θ_{u,s,v})、非角度鏈路功率因子 H_{u,s,v}(t) 與角度相關發射增益 Gᵀ(θ_{u,s,v}) 線性相乘計算。',
  'panel.formulaTerms.numeratorHint': '鏈路 RF 功率 · 非角度鏈路因子 · 發射增益 ＝ 接收訊號功率',
  'panel.formulaTerms.denominator': '分母：干擾與雜訊',
  'panel.formulaTerms.denominator.help': '分母為同一時刻與訊號競爭的功率總和：總同頻干擾 I_{u,s,v}(t, θ)（包含同衛星與跨衛星同頻波束貢獻）加上接收機熱雜訊 σ²。',
  ...
  'panel.formulaTerms.signalTotal.help': '選定連線的接收訊號功率，由 p_{u,s,v}(t, θ_{u,s,v}) · H_{u,s,v}(t) · Gᵀ(θ_{u,s,v}) 線性相乘得出，即 γ 的分子。',
  'panel.formulaTerms.txPower.label': '發射功率',
  'panel.formulaTerms.txPower.help': '此波束的實際 RF 發射功率 p_{u,s,v}(t, θ_{u,s,v})，以線性功率（W）參與主公式計算。',
  'panel.formulaTerms.txGain.label': '發射天線增益',
  'panel.formulaTerms.txGain.help': '發射天線增益以 Gᵀ(θ_{u,s,v}) = G₀F(θ_{u,s,v}, θ_{3dB}) 表示；F 是 HOBS 式 (3) 的 J₁/J₃ 角度型樣，波束中心為 1。',
  ...
  'panel.formulaTerms.pathLoss.help': '路徑損耗層以 L_{u,s,v}(t) = L_fs + L_g + L_sc + L_sf 展開，並進入 H_{u,s,v}(t)。',
  'panel.formulaTerms.intraInterference.label': '同衛星干擾',
  'panel.formulaTerms.intraInterference.help': '同一顆衛星上其他同頻波束落在此接收端的干擾貢獻，計入總同頻干擾 I_{u,s,v}(t, θ)。',
  'panel.formulaTerms.interInterference.label': '跨衛星干擾',
  'panel.formulaTerms.interInterference.help': '其他衛星的同頻波束落在此接收端的干擾貢獻，計入總同頻干擾 I_{u,s,v}(t, θ)。',
  ...
  'panel.formulaTerms.denominatorTotal.help': '總同頻干擾 I_{u,s,v}(t, θ) 與熱雜訊 σ² 的功率總和，即 γ 的分母。',
...
const LOCAL_EN: Record<string, string> = {
  ...
  'panel.formulaTerms.title': 'SINR',
  'panel.formulaTerms.help': 'γ is computed from link RF power p_{u,s,v}(t, θ_{u,s,v}), the non-angle channel factor H_{u,s,v}(t), angle-dependent transmit gain Gᵀ(θ_{u,s,v}), total co-channel interference I_{u,s,v}(t, θ), and receiver noise σ².',
  ...
  'panel.formulaTerms.numerator.help': 'The numerator is the received signal power of the selected link: computed as the linear power product of link RF power p_{u,s,v}(t, θ_{u,s,v}), non-angle channel factor H_{u,s,v}(t), and angle-dependent transmit gain Gᵀ(θ_{u,s,v}).',
  ...
  'panel.formulaTerms.denominator.help': 'The denominator is the total power competing with the signal: total co-channel interference I_{u,s,v}(t, θ) (including intra-satellite and inter-satellite co-channel beam contributions) plus receiver thermal noise σ².',
  ...
  'panel.formulaTerms.signalTotal.help': 'The received signal power of the selected link, obtained from the linear product p_{u,s,v}(t, θ_{u,s,v}) · H_{u,s,v}(t) · Gᵀ(θ_{u,s,v}), forming the numerator of γ.',
  'panel.formulaTerms.txPower.label': 'Transmit power',
  'panel.formulaTerms.txPower.help': 'The actual RF transmit power p_{u,s,v}(t, θ_{u,s,v}) of this beam, used in watts (W) as a linear power in the main formula.',
  'panel.formulaTerms.txGain.label': 'Transmit antenna gain',
  'panel.formulaTerms.txGain.help': 'Transmit gain is written as Gᵀ(θ_{u,s,v}) = G₀F(θ_{u,s,v}, θ_{3dB}), where F is the J₁/J₃ pattern of HOBS Eq. (3), unity at beam centre.',
  ...
  'panel.formulaTerms.pathLoss.help': 'The path-loss layer expands as L_{u,s,v}(t) = L_fs + L_g + L_sc + L_sf and enters H_{u,s,v}(t).',
  'panel.formulaTerms.intraInterference.label': 'Intra-satellite interference',
  'panel.formulaTerms.intraInterference.help': 'The interference contribution from other co-channel beams on the same satellite, contributing to total co-channel interference I_{u,s,v}(t, θ).',
  'panel.formulaTerms.interInterference.label': 'Inter-satellite interference',
  'panel.formulaTerms.interInterference.help': 'The interference contribution from co-channel beams on other satellites, contributing to total co-channel interference I_{u,s,v}(t, θ).',
  ...
  'panel.formulaTerms.denominatorTotal.help': 'The total power sum of co-channel interference I_{u,s,v}(t, θ) and thermal noise σ² — the denominator of γ.',
