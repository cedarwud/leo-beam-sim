...
test('one shared MainScene renderer consumes the plan only as a mount gate', () => {
  ...
  assert.match(mainSceneSource, /presentationPlan\.visible\[['"]load-overlays['"]\] && beamLoadContentionEnabled/);
  assert.match(mainSceneSource, /presentationPlan\.visible\[['"]event-effects['"]\][\s\S]*u\.isOtherHandover === true/);
