...
export class HandoverManager {
  ...
  update(
    ...
    if (this.state.satId === null) {
      ...
      if (best.sinrDb >= attachThresholdDb) {
        return this.commitDecision(
          ...
          mintInitialAttachPermit('manager:initial-attach'),
        );
      }
    ...
    if (currentSinr === -Infinity) {
      ...
      if (rescue) {
        return this.commitDecision(
          ...
          mintLegacyEeBlindPermit('manager:continuity-rescue'),
        );
      }
    }
    ...
    if (bestTarget) {
      ...
      if (sameAsPending) {
        ...
        if (this.state.triggerTimeSec >= this.triggerTimeSec) {
          return this.commitDecision(
            ...
            mintLegacyEeBlindPermit('manager:inter-stable-pending-hold'),
          );
        }
      ...
      if (activePendingSample && this.state.triggerTimeSec >= this.triggerTimeSec) {
        return this.commitDecision(
          ...
          mintLegacyEeBlindPermit('manager:inter-stable-target'),
        );
      }
    ...
    if (this.intraSwitchTarget.triggerTimeSec >= this.intraSwitchTimeSec) {
      return this.commitDecision(
        ...
        mintLegacyEeBlindPermit('manager:intra-dwell'),
      );
    }
  ...
  private commitDecision(
    ...
    simTimeMs: number,
    reason: string,
    // Authorization to commit, not a description of the commit. A path that
    // cannot obtain a permit cannot reach this function: the parameter is
    // required, so a newly added commit path fails to compile until it states
