...
export const ZH_TW = {
  ...
  'kpi.cumulativeDeliveredData.help':
    '自模擬開始累積送出的資料總量：ΣMbit ＝ Σ R(t)·Δt，即各時間步的吞吐量乘上該步的時間長度後加總，單位為 Mbit。',

  'kpi.cumulativeEnergy.label': '累積耗能',
  'kpi.cumulativeEnergy.help':
    '自模擬開始累積消耗的能量總量：ΣJ ＝ Σ P_total(t)·Δt，即各時間步的總功率乘上該步的時間長度後加總，單位為焦耳（J）。',
...
export const EN: Record<I18nKey, string> = {
  ...
  'kpi.cumulativeDeliveredData.help':
    "The total data delivered since the run started: ΣMbit = Σ R(t)·Δt — each step's throughput multiplied by that step's duration, summed, in megabits (Mbit).",

  'kpi.cumulativeEnergy.label': 'Cumulative energy',
  'kpi.cumulativeEnergy.help':
    "The total energy consumed since the run started: ΣJ = Σ P_total(t)·Δt — each step's total power multiplied by that step's duration, summed, in joules (J).",
