    ...
    <div className="simulator-panel-grid">
      <Section title="可編輯的 canonical power inputs" subtitle="這五個欄位是限制條件／功耗模型參數；P_DL_actual 與 PA 功率不是輸入欄位。">
        <div className="simulator-form-grid">
          ...
          <NumberField id="rfc-power" label="P_RFC（RF chain）" unit="W / active beam" value={parameters.rfcPowerW} min={0} step={0.01} description="每個 active beam 分攤的 RF-chain 功率。" source="Power ledger；隨 active beam 計入。" onChange={update('rfcPowerW')} />
          <NumberField id="bb-power" label="P_BB（baseband）" unit="W / satellite" value={parameters.basebandPerSatelliteW} min={0} step={0.01} description="衛星 baseband 功率，按 active beam 分攤。" source="Power ledger；不是 RF output。" onChange={update('basebandPerSatelliteW')} />
        </div>
