...
export const VISUAL_LAB_INPUT_SUBGROUPS: readonly VisualLabInputSubgroupDefinition[] = [
  ...
  },
  {
    ...
    description: localized('熱雜訊、接收器雜訊與頻率重用。', 'Thermal noise, receiver noise and frequency reuse.'),
    copy: copyBundle(
      localized('雜訊與干擾', 'Noise and interference'),
      localized('熱雜訊、接收器雜訊與頻率重用。', 'Thermal noise, receiver noise and frequency reuse.'),
    ),
    searchTerms: searchTerms(['雜訊', '干擾', '頻率重用', 'K_FR'], ['noise', 'interference', 'frequency reuse', 'K_FR']),
  },
  {
    ...
    description: localized('系統頻寬與最低服務速率目標。', 'System bandwidth and minimum service-rate target.'),
    copy: copyBundle(
      localized('頻寬與服務需求', 'Bandwidth and service demand'),
      localized('系統頻寬與最低服務速率目標。', 'System bandwidth and minimum service-rate target.'),
    ),
    searchTerms: searchTerms(['頻寬', '服務需求', '最低速率'], ['bandwidth', 'service demand', 'minimum rate']),
  },
  {
    key: 'limits',
    group: 'power',
...
export const VISUAL_LAB_INPUT_DEFINITIONS: readonly VisualLabInputDefinition[] = [
  definition({
    ...
    causalPath: localized('K_FR → B_beam、I → SINR／速率／EE', 'K_FR → B_beam, I → SINR / rate / EE'),
  ...
  definition({
    ...
    causalPath: localized('T_ant → T_sys → noise → SINR／速率／EE', 'T_ant → T_sys → noise → SINR / rate / EE'),
  ...
  definition({
    ...
    causalPath: localized('NF → T_sys → noise → SINR／速率／EE', 'NF → T_sys → noise → SINR / rate / EE'),
    provenance: localized('canonical physical-noise construction。', 'Canonical physical-noise construction.'),
  ...
  definition({
    ...
    causalPath: localized('T_0、NF → T_sys → noise → SINR／速率／EE', 'T_0, NF → T_sys → noise → SINR / rate / EE'),
    provenance: localized('canonical physical-noise construction。', 'Canonical physical-noise construction.'),
  ...
  definition({
    ...
    causalPath: localized('G_0 → G_T(θ) → h → RF／SINR／速率／EE', 'G_0 → G_T(θ) → h → RF / SINR / rate / EE'),
    provenance: localized('canonical G_0；dBi 僅為顯示單位。', 'Canonical G_0; dBi is a display unit only.'),
    searchTerms: searchTerms(['波束中心增益', 'G_0', 'dBi', '發射增益'], ['boresight gain', 'G_0', 'dBi', 'transmit gain']),
  ...
  definition({
    ...
    causalPath: localized('theta_3dB → G_T(θ) → h → RF／SINR／速率／EE', 'theta_3dB → G_T(θ) → h → RF / SINR / rate / EE'),
    provenance: localized('canonical theta_3dB；degree 僅為顯示單位。', 'Canonical theta_3dB; degrees are a display unit only.'),
    searchTerms: searchTerms(['3 dB 波束寬度', 'theta_3dB', '波束寬度'], ['3 dB beamwidth', 'theta_3dB', 'beamwidth']),
  ...
  definition({
    ...
    causalPath: localized('f_c → G^LS → h → RF／SINR／速率／EE', 'f_c → G^LS → h → RF / SINR / rate / EE'),
    provenance: localized('正式 canonical channel adapter。', 'Formal canonical channel adapter.'),
    searchTerms: searchTerms(['載波頻率', 'f_c', 'GHz'], ['carrier frequency', 'f_c', 'GHz']),
  ...
  definition({
    ...
    causalPath: localized('chi_atm → G^LS → h → RF／SINR／速率／EE', 'chi_atm → G^LS → h → RF / SINR / rate / EE'),
    provenance: localized('正式 canonical channel adapter。', 'Formal canonical channel adapter.'),
  ...
  definition({
    ...
    causalPath: localized('G^R → h → RF／SINR／速率／EE', 'G^R → h → RF / SINR / rate / EE'),
    provenance: localized('正式 canonical receive-gain adapter。', 'Formal canonical receive-gain adapter.'),
    searchTerms: searchTerms(['接收天線增益', 'G^R', 'dBi'], ['receive antenna gain', 'G^R', 'dBi']),
  ...
  definition({
    ...
    causalPath: localized('P_beam,max → P_DL_actual → SINR／速率／P_sys／EE', 'P_beam,max → P_DL_actual → SINR / rate / P_sys / EE'),
  ...
  definition({
    ...
    causalPath: localized('P_sat,max → P_DL_actual → SINR／速率／P_sys／EE', 'P_sat,max → P_DL_actual → SINR / rate / P_sys / EE'),
    provenance: localized('canonical power boundary。', 'Canonical power boundary.'),
  ...
  definition({
    ...
    causalPath: localized('R_min → gamma_req → p_req → RF／速率／EE', 'R_min → gamma_req → p_req → RF / rate / EE'),
    provenance: localized('canonical service target。', 'Canonical service target.'),
    searchTerms: searchTerms(['最低傳輸速率', 'R_min', '服務品質'], ['minimum rate', 'R_min', 'service quality']),
  ...
  definition({
    ...
    description: localized('系統總頻寬；每波束頻寬由 K_FR 推導。', 'Total system bandwidth; per-beam bandwidth follows K_FR.'),
    causalPath: localized('B_sys、K_FR → B_beam、noise／速率／EE', 'B_sys, K_FR → B_beam, noise / rate / EE'),
  ...
  definition({
    ...
    causalPath: localized('eta_max、BO → eta_PA → P_sys → energy／EE', 'eta_max, BO → eta_PA → P_sys → energy / EE'),
  ...
  definition({
    ...
    causalPath: localized('BO、eta_max → eta_PA → P_sys → energy／EE', 'BO, eta_max → eta_PA → P_sys → energy / EE'),
    provenance: localized('canonical PA model。', 'Canonical PA model.'),
  ...
  definition({
    ...
    causalPath: localized('P_RFC × active beams → P_sys → energy／EE', 'P_RFC × active beams → P_sys → energy / EE'),
    provenance: localized('canonical power ledger。', 'Canonical power ledger.'),
    searchTerms: searchTerms(['RF chain 功率', 'P_RFC', '啟用波束'], ['RF-chain power', 'P_RFC', 'active beam']),
  ...
  definition({
    ...
    causalPath: localized('P_BB × active satellites → P_sys → energy／EE', 'P_BB × active satellites → P_sys → energy / EE'),
    provenance: localized('canonical power ledger。', 'Canonical power ledger.'),
    searchTerms: searchTerms(['baseband 功率', 'P_BB', '啟用衛星'], ['baseband power', 'P_BB', 'active satellite']),
