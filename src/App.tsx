...
// ITEM #C: lead band for the direction-aware live Director seek-landing check.
...
export function App() {
  ...
  // handleSimUpdate) used as "now" when resolving the next handover event, and
  // the deferred focus armed by the live Director seek (fired once the seek
  // lands so the sat-pair pose is computed against the post-seek scene frame).
  ...
  useEffect(() => {
    ...
    const dtSec = simState.simTimeSec - pending.seekTargetSec;
  ...
  }, [simState.simTimeSec, camera, camera.directorPhase]);
