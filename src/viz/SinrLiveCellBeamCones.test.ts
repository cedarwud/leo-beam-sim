 ...
 * (S-cells-3). Authority `docs/sinr-live-earth-fixed-cells-mini-sdd.md` §5.5 +
 * decisions A1/B3.
 *
 * This locks the PURE resolver contract — the cone geometry, not the cell physics
 * (that is `validate:phase-c:sinr-live-cells:model`) and not the runtime wiring
 * (that is `:runtime`). It asserts the milestone invariants:
 *   1. one cone per SERVED cell; idle cells (servingSatId === null) draw no cone;
 *   2. cone base sits at the FIXED cell centre (placement), NOT on the UE — this
 *      is the whole "UE off-centre" point;
 ...
 *   5. serving-satellite count + render count helpers agree with the items;
 *   6. the resolver consumes the cell TRUTH only — never the round-robin
 *      `useCellSchedule` / `cellScheduler` (codex BLOCK-3): asserted structurally.
 *
...
// them at the world origin (where the static primary UE sits) — so a cone never
// coincides with the UE.
...
check('cone base = FIXED cell centre (NOT the UE/origin), apex = serving sat', () => {
  const items = resolveSinrLiveCellBeamConeItems({
  ...
  approx(c.baseCenter.y, 0, 1e-9, 'base on the ground plane');
  // The static primary UE sits at the world origin; the cone base must be off it.
  ...
  assertEqual(c.color, '#ff0000', 'cone tinted by serving sat');
});
...
check('quaternion tilts the cone toward the cell (sat→cell axis, real 波束變型)', () => {
  const items = resolveSinrLiveCellBeamConeItems({
    cellFrame: frameOf([cell(2, 'sat-B')]),
    placementByCellId,
    satelliteWorldById,
    satelliteTintById,
  });
  const c = items[0];
  // The local base axis (0,-1,0) rotated by the quaternion must point sat→cell.
  const rotated = new THREE.Vector3(0, -1, 0).applyQuaternion(c.quaternion);
  const expected = c.baseCenter.clone().sub(c.apex).normalize();
  approx(rotated.x, expected.x, 1e-6, 'rotated axis x = sat→cell');
  approx(rotated.y, expected.y, 1e-6, 'rotated axis y = sat→cell');
  approx(rotated.z, expected.z, 1e-6, 'rotated axis z = sat→cell');
});

// (Structural purity — the resolver never importing the round-robin scheduler,
// codex BLOCK-3 — is locked in `validate:frontend:scene-lane-governance`, which
// has filesystem access; this model gate stays node-types-free.)
