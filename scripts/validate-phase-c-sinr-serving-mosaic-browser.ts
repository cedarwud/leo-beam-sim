  ...
  try {
    ...
    const snapshotHandle = await page.waitForFunction(
      () => {
        ...
        if (
          total > 1 && served > 1 && served <= total && beams > 1
          && mesh > 1 && Number.isFinite(avg) && loadRows >= 1
        ) {
          return { served, total, beams, mesh, avg, loadRows };
        }
        return false;
      },
      undefined,
    ...
    const snap = (await snapshotHandle.jsonValue()) as {
      served: number; total: number; beams: number; mesh: number; avg: number; loadRows: number;
    };

    assert.ok(snap.total > 1, `population is multi-UE (total=${snap.total})`);
    ...
    console.log('[sinr-serving-mosaic] PASS — ambient mosaic + aggregate on sinr-live (served N/N, multi-beam partition, finite mean SINR, mesh-derived colour count > 1, claim=sinr-serving); absent on the MODQN lane (DATA SOURCE = live Walker SINR)');
  } finally {
    await browser.close();
  }
}
